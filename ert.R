# ==== ERT (ranger extratrees, mean) — paper-style pipeline ====
...
# Saves: forecasts/ert/ERT_cpi_total.csv and ERT-list-cpi_total.Rdata
# With importance = "impurity_corrected" saved per (h,k) in ERT_cpi_list[[h]]$varimp
...
# ---------------- Model helper: one (h,k) forecast with ERT (mean) ----------
ert_one_k <- function(k, Xall, yall, dates, idx_oos, h, R,
  ...
  # Edge-case: near-constant design → mean fallback
  if (nrow(unique(Xs)) <= 1L) {
    ...
    return(list(pred = yhat_mean,
                ...
                debug = if (h == 1 && k == length(idx_oos)) list(
                  ...
                  note = "Xs had <=1 unique row; mean used") else NULL))
  }
  ...
  repeat {
    ...
    if (grepl("sample larger than population", msg, ignore.case = TRUE)) {
      if (mtry_try > 1L) { mtry_try <- max(1L, floor(mtry_try / 2)); next }
    }
    yhat_mean <- mean(y_tr)
    return(list(pred = yhat_mean,
                varimp = setNames(numeric(p), colnames(Xs)),
  ...
  yhat <- as.numeric(predict(fit, as.data.frame(xo))$predictions)  # mean prediction
  
  vi <- fit$variable.importance; if (!is.null(vi)) names(vi) <- colnames(Xs)
...
ERT_cpi_list <- vector("list", length(horizons))

# Parallel backend (parallelize across k; keep ranger num.threads=1 inside workers)
cl <- NULL
if (use_parallel) {
...
for (h in horizons) {
  # Freeze PCs on the initial training window (ending at origin_first)
  ...
  if (use_parallel) {
    res_list <- foreach(k = 1:nprev,
                        .packages = c("ranger"),
                        .export = c("ert_one_k","standardize_impute","build_xy_paper","compute_metrics"),
                        .options.RNG = 20240601) %dorng% {
                          ert_one_k(k, Xall, yall, dates, idx_oos, h, R,
    ...
    if (!is.null(res_list[[nprev]]$debug)) {
      ...
      cat(sprintf(
        "\n[h=1, k=last] origin_k=%s last_train_anchor_k=%s mtry_used=%d ntree=%d min_node=%d splitrule=%s num.random.splits=%d attempts=%d yhat=%.4f\n",
        d$origin_k, d$last_train_anchor_k, as.integer(d$mtry_used), as.integer(d$ntree),
        as.integer(d$min_node), d$splitrule, as.integer(d$num_random_splits),
        as.integer(d$attempts), as.numeric(d$yhat)
      ))
    }
  } else {
    preds <- numeric(nprev)
    ...
    for (k in 1:nprev) {
      outk <- ert_one_k(k, Xall, yall, dates, idx_oos, h, R,
                        cap = winsor_cap, ntree = 3000, mtry_frac = 1/3,
                        min_node = 8, num_random_splits = 5, rng_seed = 12345)
      preds[k] <- outk$pred
  ...
  ERT_cpi_list[[h]] <- list(
    ...
    varimp   = varimp_list   # per-k corrected-impurity VI (no aggregation)
  )
}
...
mat_ert <- do.call(cbind, lapply(1:12, function(h) ERT_cpi_list[[h]]$pred))
colnames(mat_ert) <- paste0("h", 1:12)
rownames(mat_ert) <- dates_oos

outfile_csv  <- file.path("forecasts", model_folder, "ERT_cpi_total.csv")
outfile_rdat <- file.path("forecasts", model_folder, "ERT-list-cpi_total.Rdata")

write.table(mat_ert, outfile_csv, sep = ";", row.names = TRUE, col.names = TRUE)
save(ERT_cpi_list, file = outfile_rdat)

cat("Saved ERT (ranger extratrees mean; impurity_corrected VI) to forecasts/",
