...
mcs_membership <- function(loss_mat_full, type = c("sq","ab")){
  ...
  memb <- rep(NA, sum(keep_cols))
  if (ncol(M) >= 2) {
    out <- NULL
    suppressWarnings(capture.output({
      out <- MCS::MCSprocedure(Loss = as.data.frame(M), alpha = 0.10, B = 5000,
    ...
    if (!is.null(out)) {
      ...
      if (!is.null(keep_models)) {
        memb <- colnames(M) %in% keep_models
      } else {
        keep_models <- extract_mcs_membership(out, which_col = "auto")
        memb <- colnames(M) %in% keep_models
      }
    } else {
      memb <- rep(NA, ncol(M))
